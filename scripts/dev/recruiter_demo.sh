...
write_validation_receipt() {
  ...
  python - <<'PY' "$receipt_path" "$command_text"
import json
import sys
...
main() {
  ...
  CURRENT_STEP="integration_and_async_baseline_validation"
  log "running integration tests and async pipeline baseline checks"
  run_checked \
    "integration and async pipeline baseline validation failed" \
    pytest -m "integration or perf" -q
  write_validation_receipt "integration_and_async_baseline_validation" \
    "pytest -m \"integration or perf\" -q"
  ...
  run_checked \
    ...
    --validation-receipt "integration_and_async_baseline_validation=build/reviewer-validations/integration_and_async_baseline_validation.json" \
...
4. Integration validations plus async pipeline baseline checks (\`pytest -m "integration or perf"\`)
5. Deterministic MLOps evidence generation (\`build/recruiter-ml-evidence.json\`)
6. Reviewer scorecard generation from validation receipts (\`build/reviewer-scorecard.md\`)
